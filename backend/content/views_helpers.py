...
def annotate_likes_queryset(queryset, request=None):
    ...
    if request and getattr(request, "user", None) and request.user.is_authenticated:
        try:
            ...
            likes_subq = Like.objects.filter(
                content_type=ct, object_id=OuterRef("pk"), user=request.user
            )
...
class BaseContentViewSet(viewsets.ModelViewSet):
    ...
    @action(
        detail=False, methods=("get",), url_path="top-viewed", url_name="top_viewed"
    )
    ...
    @action(
        detail=False,
        methods=("get",),
        url_path="top-commented",
        url_name="top_commented",
    )
