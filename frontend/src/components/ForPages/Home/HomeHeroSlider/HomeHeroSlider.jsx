...
export default function HomeHeroSlider({ t, fullLiveStream }) {
    ...
    <div className="w-full relative h-[50vh] sm:h-[60vh] md:h-[70vh] lg:h-[92vh]">
      <div className="relative w-full h-full overflow-hidden shadow-2xl group">
        ...
        <div className="absolute inset-0 bg-black">
          <img
            src={`${fullLiveStream?.thumbnail_url?.maxres?.url || fullLiveStream?.thumbnail_url?.standard?.url || "/assets/default-thumbnail.jpg"}`}
            alt={fullLiveStream?.title || "Video Thumbnail"}
            className="absolute inset-0  w-full aspect-[16/9] object-cover transition-transform duration-500 group-hover:scale-105"
          />
          ...
          <button
            onClick={() => navigate(`/videos/${fullLiveStream.id}`)}
          ...
          <div className="absolute top-1/3 lg:top-1/2 left-4 sm:left-6 md:left-8 lg:left-32 transform -translate-y-1/2 flex flex-col p-4 sm:p-6 md:p-8 max-w-xs sm:max-w-sm md:max-w-lg lg:max-w-full">
            ...
            <button
              onClick={() => {
                navigate(`/videos/${fullLiveStream?.id}`);
              }}
            ...
            >
              ...
              <span className="text-primary text-xl ">{t("Watch")}</span>
            </button>
            {/* End Watch Button */}
          </div>
        </div>
