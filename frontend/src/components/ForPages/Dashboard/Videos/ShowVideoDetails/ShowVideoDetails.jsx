          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">{selectedVideo.duration || 'N/A'}</span>
              </div>
              <div className="flex items-center gap-2">
                <Eye className="w-4 h-4 text-green-500" />
                <span className="text-gray-600">{t("Views")}: </span>
                <span className="font-medium">{formatViews(selectedVideo.views)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <span className="text-gray-600 text-sm block mb-1">{t("Category")}</span>
                <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">
                  {selectedVideo.category || 'N/A'}
                </span>
              </div>
              <div>
                <span className="text-gray-600 text-sm block mb-1">{t("Video Type")}</span>
                <span className="bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm font-medium">
                  {selectedVideo.video_type || 'N/A'}
                </span>
              </div>
              <div>
                <span className="text-gray-600 text-sm block mb-1">{t("Language")}</span>
                <span className="font-medium text-gray-800">{selectedVideo.language || 'N/A'}</span>
              </div>
            </div>
            <div className="space-y-4">
              <div>
                <span className="text-gray-600 text-sm block mb-1">{t("Subject")}</span>
                <span className="font-medium text-gray-800">{selectedVideo.subject || 'N/A'}</span>
              </div>
              <div>
                ...
                <span className="font-medium text-gray-800">{formatDate(selectedVideo.published_at)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-6">
          ...
          {selectedVideo.video_url && (
            <div className="bg-gray-50 p-4 rounded-lg">
              <span className="text-gray-600 text-sm block mb-2">{t("Video URL")}</span>
              <a
