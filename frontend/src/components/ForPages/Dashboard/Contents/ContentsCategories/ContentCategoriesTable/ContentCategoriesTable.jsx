...
import { EditContentCategoryById, SortContentCategories } from "@/api/contents";
import DraggableTable from "@/components/ForPages/Dashboard/DraggableTable/DraggableTable";

function ContentCategoriesTable({
  t,
  i18n,
  categories,
  setCategories,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item) => (
        <button
          onClick={async () => {
            if (!item.is_active && item.content_count === 0) {
              toast.info(
                t(
            ...
            try {
              await EditContentCategoryById(item.id, {
                ...item,
          ...
          className={`p-1 rounded-lg transition-all duration-200 ${
            ...
            !item.is_active && item.content_count === 0
              ? "opacity-50 "
              : ""
          }`}
          title={
...
export default ContentCategoriesTable;