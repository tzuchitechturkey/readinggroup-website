...
function TvCategoriesContent() {
  ...
  const getCategoriesData = async (page) => {
    ...
    try {
      const res = await GetTvCategories(limit, offset);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCategory && editingCategory.id) {
        await EditTvCategoryById(editingCategory.id, form);
        toast.success(t("Category updated"));
      } else {
        await AddTvCategory(form);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await DeleteTvCategory(selectedCategory.id);
  ...
  return (
    <div className="bg-white rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-medium">{t("TV Categories")}</h3>
        <button
...
export default TvCategoriesContent;
