...
const VideoClipCard = ({ video, size = "default", showDate = false }) => {
  const getSizeClasses = () => {
    switch (size) {
      case "large":
        return {
          container: "w-full sm:w-[95%] md:w-[90%] lg:w-[792px] h-auto sm:h-[200px] md:h-[300px] lg:h-[446px]",
          image: "h-[200px] sm:h-[200px] md:h-[300px] lg:h-[446px] w-full sm:w-[95%] md:w-[90%] lg:w-[792px]",
          titleHeight: "h-[30px] sm:h-[32px] md:h-[34px] lg:h-[35px]",
          categoryText: "text-[14px] sm:text-[15px] md:text-[16px] lg:text-[18px]",
          durationText: "text-[13px] sm:text-[14px] md:text-[15px] lg:text-[16px]",
          gap: "gap-[8px] sm:gap-[10px] md:gap-[12px] lg:gap-[12px]",
        };
      case "small":
        return {
          container: "w-full sm:w-[48%] md:w-[45%] lg:w-[354px] h-auto sm:h-[120px] md:h-[160px] lg:h-[199px]",
          image: "h-[150px] sm:h-[120px] md:h-[160px] lg:h-[199px] w-full sm:w-[48%] md:w-[45%] lg:w-[354px]",
          titleHeight: "h-[25px] sm:h-[26px] md:h-[28px] lg:h-[30px]",
          categoryText: "text-[12px] sm:text-[13px] md:text-[14px] lg:text-[18px]",
          durationText: "text-[12px] sm:text-[13px] md:text-[14px] lg:text-[16px]",
  ...
  return (
    <div className="flex flex-col gap-[3px] sm:gap-[4px] md:gap-[4px] lg:gap-[4px] items-start w-full">
      ...
      <div className={`flex ${gap} items-center w-full mt-0.5 sm:mt-1 mx-0.5 sm:mx-1`}>
        <p className={`font-['Noto_Sans_TC:Regular',sans-serif] font-normal leading-[1.5] ${categoryText} text-black uppercase`}>
          {video.category}
        </p>
        <div className="h-[12px] sm:h-[14px] md:h-[15px] lg:h-[16px] w-0 border-r border-gray-400" />
        <p className={`font-['Noto_Sans_TC:Regular',sans-serif] font-normal leading-[1.5] ${durationText} text-black`}>
          {video.duration}
        </p>
      </div>
      {showDate && (
        <div className="mx-1 -mt-1">
          <p className="text-[11px] sm:text-[12px] md:text-[13px] lg:text-[14px]">{video?.date || "2012-23-32"}</p>
        </div>
      )}
    </div>
  );
