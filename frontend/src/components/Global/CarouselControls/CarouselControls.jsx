  ...
  return (
    <div className="flex items-center justify-center gap-3 sm:gap-3 md:gap-4 lg:gap-4 mt-3 sm:mt-3 md:mt-4 lg:mt-4">
      ...
      <button
        ...
        onClick={onPrev}
        disabled={!canPrev}
        className={`p-0.5 sm:p-0.5 md:p-1 lg:p-1 rounded-full transition-all duration-200 ${canPrev ? `hover:scale-110 active:scale-95 ${buttonClass}` : `opacity-40 cursor-not-allowed ${buttonClass}/50`
          }`}
        aria-label="Previous slide"
      >
        <ChevronLeft className="w-4 sm:w-4 md:w-5 lg:w-5 h-4 sm:h-4 md:h-5 lg:h-5" />
      </button>
      ...
      <div className="flex gap-2 sm:gap-2 md:gap-2.5 lg:gap-3 items-center min-h-[12px] px-2">
        {Array.from({ length: count }).map((_, index) => (
          <button
            ...
            className={`transition-all duration-300 rounded-full flex-shrink-0 ${current === index
                ? `${activeClass} w-8 h-2 sm:w-8 sm:h-2 md:w-8 md:h-2 lg:w-8 lg:h-2`
                : `${inactiveClass} w-2 h-2 sm:w-2 sm:h-2 md:w-2 md:h-2 lg:w-2 lg:h-2 hover:opacity-80`
              }`}
      ...
      <button
        ...
        onClick={onNext}
        disabled={!canNext}
        className={`p-0.5 sm:p-0.5 md:p-1 lg:p-1 rounded-full transition-all duration-200 ${canNext ? `hover:scale-110 active:scale-95 ${buttonClass}` : `opacity-40 cursor-not-allowed ${buttonClass}/50`
          }`}
